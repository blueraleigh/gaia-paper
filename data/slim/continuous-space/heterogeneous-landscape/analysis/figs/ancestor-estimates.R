...
y = read.csv("../ancestor-estimates-pareto.csv", header=FALSE)

png(file="ancestor-estimates.png", width=6.5, height=3.5, units="in", res=300)
par(oma=c(4,4,0,0), mar=c(1,1,1,1))
layout(matrix(1:2, 1, 2))
plot(x[,3], x[,4], log='x', las=1, bty="l",
    xlab="Ancestor age",
    ylab="Ancestor location error",
    type="n", xaxt="n", yaxt="n", ylim=c(0,0.5))
...
l = numeric(9)
b = c(1,5,10,50,100,500,1000,5000,10000)
for (i in 1:9)
    l[i] = quantile(x[x[,3] <= b[i],4], 0.95)
lines(b, l, lty=2)
...
plot(y[,3], y[,4], log='x', las=1, bty="l",
    ...
    type="n", xaxt="n", yaxt="n", ylim=c(0,0.5))
axis(1, at=c(1,10,100,1000,10000,''))
axis(2, las=1, at=seq(0,0.5,0.1), labels=rep('', 6))
mtext("Ancestor age (generations)", 1, line=2.5, cex=0.8)
myhsp(y[,3], y[,4], colpal="heat", log='x',pch=19)
l = numeric(9)
b = c(1,5,10,50,100,500,1000,5000,10000)
for (i in 1:9)
    l[i] = quantile(y[y[,3] <= b[i],4], 0.95)
lines(b, l, lty=2)
